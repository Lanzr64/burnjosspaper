package net.lanzr.burnjosspaper.menu;

import com.google.common.collect.Sets;
import com.ibm.icu.impl.number.range.PrefixInfixSuffixLengthHelper;
import com.mojang.logging.LogUtils;
import net.lanzr.burnjosspaper.container.FixedSlot;
...
public class PaginationContainer extends AbstractContainerMenu {
    ...
    /** 每页物品格数 (去掉左下/右下导航按钮) */
    public static final int ITEM_SLOTS_PER_PAGE = DISPLAY_SLOTS - 2; // 52
    ...
    private void loadCurrentPage() {
        if (currentPage == 1) {
            navItemPrev.setHoverName(Component.literal("§7◀ 已到最前"));
        } else {
            navItemPrev.setHoverName(Component.literal("§a◀ 上一页"));
        }

        if (currentPage == maxPages) {
            navItemNext.setHoverName(Component.literal("§7已到最后 ▶"));
        } else {
            navItemNext.setHoverName(Component.literal("§a下一页 ▶"));
        }

        indexItem.setHoverName(Component.literal(String.format("%d / %d", currentPage, maxPages)));
    }
...
//            do {
//                if (slot instanceof DynamicPageSlot dynSlot && (clickType == ClickType.PICKUP) && dynSlot.isInPageRange()) {
//                    ItemStack carried = getCarried();
//                    ItemStack inSlot = dynSlot.getItem();
//                    if (dragType == 0) {
//                        // 左键
//                        if (carried.isEmpty() && !inSlot.isEmpty()) {
//                            // 拿起全部
//                            setCarried(inSlot.copy());
//                            dynSlot.set(ItemStack.EMPTY);
//                            break;
//
//                        } else if (!carried.isEmpty() && inSlot.isEmpty()) {
//                            // 放下全部
//                            dynSlot.set(carried.copy());
//                            setCarried(ItemStack.EMPTY);
//                            break;
//                        } else if (!carried.isEmpty() && !inSlot.isEmpty()) {
//                            if (ItemStack.isSameItemSameTags(carried, inSlot)) {
//                                // 同类物品 → 堆叠合并
//                                int maxSize = Math.min(dynSlot.getMaxStackSize(), carried.getMaxStackSize());
...
//                                    System.out.println("pick success");
                                ...
                                if (ItemStack.isSameItemSameTags(dynstack, carrystack)) {
                            ...
                            } else if (ItemStack.isSameItemSameTags(dynstack, carrystack)) {
